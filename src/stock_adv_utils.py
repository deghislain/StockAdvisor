...
def filter_necessary_additional_info_data(data: Dict[str, Any]) -> pd.DataFrame:
    ...
    fundamental_data = {key: data.get(key) for key in ADDITIONAL_INFO_KEYS}

    # Check for missing required data and raise a ValueError if needed (optional)
    if any(value is None for value in fundamental_data.values()):
...
def filter_by_year(stock_data: pd.DataFrame, keys: List[str]) -> pd.DataFrame:
    ...
    for year in DATA_DATES:
        try:
            ...
            if current_stock_data:
                filtered_stock_data[year] = current_stock_data
...
def filter_necessary_fundamental_data(stock_data: pd.DataFrame, keys: List[str]) -> pd.DataFrame:
    logging.info(f"-----------------------------filter_necessary_fundamental_data START")
    return filter_by_year(stock_data, keys)
